...
cat ./.drive_token
curl -v -XPOST "https://www.googleapis.com/upload/drive/v3/files?uploadType=media&name=opportunities$(date +%s).zip" -H "Content-Type: application/zip, application/octet-stream" -H "Authorization: Bearer $(cat ./.drive_token)" -d@opportunities.zip
