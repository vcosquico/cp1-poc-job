...
curl -v -XPOST "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart" -H "Authorization: Bearer $(cat ./.drive_token)" -F "file=@./opportunities.zip" -F "metadata={name : 'opportunities.zip'};type=application/json;charset=UTF-8" -F "metadata={parents : '[{kind:'drive#parentReference',id:'1W-wTdGs6duUDWvGfZ0B7C3itO-W5mU_V'}]'}"
