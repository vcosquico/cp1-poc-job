...
curl -sS -XPOST "https://www.googleapis.com/upload/drive/v3/files?uploadType=media" -H "Content-Type: application/zip, application/octet-stream" -H "Authorization: Bearer $(cat ./.drive_token)" -F "file=@opportunities.zip;type=application/zip" -F "metadata={name : 'opportunities.zip'};type=application/json;charset=UTF-8"
