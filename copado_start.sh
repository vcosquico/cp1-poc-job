...
cat ./.drive_token
# put file metadata
curl -sD - -XPOST "https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&part=snippet" -H "Content-Type: application/json" -H "Authorization: Bearer $(cat ./.drive_token)" -d '{"name":"opportunities'-"$(date +%s)"'.zip"}' | grep x-guploader-uploadid:  | tr -d '\r' | sed -En 's/^x-guploader-uploadid: (.*)/\1/p' | tee ./.fileid 
# put file
